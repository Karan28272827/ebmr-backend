...
export class QcService {
  ...
  async findAll(stage?: string, status?: string, batchId?: string) {
    return this.prisma.qCTest.findMany({
      where: {
        ...(stage ? { qc_stage: stage as any } : {}),
  ...
  async submitResults(
    ...
    const resultsData = results.map((r) => {
      ...
      if (!isNaN(numVal)) {
        if (r.min_value !== undefined && numVal < r.min_value) is_within_spec = false;
        if (r.max_value !== undefined && numVal > r.max_value) is_within_spec = false;
      }
  ...
  async recordVerdict(testId: string, user: any, verdict: string, notes?: string) {
    if (!hasMinRole(user.role, 'QA_REVIEWER')) {
      throw new ForbiddenException('Only QA_REVIEWER+ can record verdicts');
    }
    ...
    if (test.material_receipt_id) {
      ...
      await this.prisma.materialReceipt.update({
        ...
        data: { qc_status: receiptStatus as any },
      });
    }
  ...
  async getDashboard() {
    ...
    const passRate =
      recentTests.length > 0 ? Math.round((passCount / recentTests.length) * 100) : 0;
  ...
  async findAllChecklists(stage?: string) {
    return this.prisma.qCChecklist.findMany({
      where: stage ? { qc_stage: stage as any } : {},
  ...
  async createChecklist(user: any, data: any) {
    if (!hasMinRole(user.role, 'QA_MANAGER')) {
      throw new ForbiddenException('Only QA_MANAGER+ can create checklists');
    }
  ...
  async addChecklistItems(checklistId: string, user: any, items: any[]) {
    if (!hasMinRole(user.role, 'QA_MANAGER')) {
      throw new ForbiddenException('Only QA_MANAGER+ can add checklist items');
    }
    const cl = await this.findChecklist(checklistId);
    if (!cl) throw new NotFoundException('Checklist not found');

    await this.prisma.checklistItem.createMany({
      data: items.map((item: any) => ({ ...item, checklist_id: checklistId })),
    });

    return this.findChecklist(checklistId);
  }

  async startChecklistExecution(user: any, checklistId: string, qcTestId: string) {
  ...
  async updateChecklistItem(
    ...
    itemId: string,
    user: any,
    data: { is_completed: boolean; value_recorded?: string; notes?: string },
  ) {
  ...
  async completeChecklistExecution(executionId: string, user: any) {
    ...
    const completedMandatory = exec.item_responses.filter(
      (r) => r.is_completed && mandatoryItems.some((m) => m.id === r.item_id),
    ).length;

    if (completedMandatory < mandatoryItems.length) {
      throw new BadRequestException(
        `${mandatoryItems.length - completedMandatory} mandatory items not completed`,
      );
    }
